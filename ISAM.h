...
struct PageData{
    RecordTipo records[N];
    size_t count;
    long long nextPage = -1;

    PageData<RecordTipo>() : count(0) {}
...
template<typename RecordTipo>
class ISAM : public SeleccionarMetodo<RecordTipo>{
public:
    ISAM(const string& csv_path,bool tipo){


        if(tipo){
        data_path="data_musica.bin" ;
        // Rutas de índice
        index_paths.push_back("index1_musica.bin");
        index_paths.push_back("index2_musica.bin");
        index_paths.push_back("index3_musica.bin");
        }
        else{
                    data_path="data_bike.bin";
                    // Rutas de índice
        index_paths.push_back("index1_bike.bin");
        index_paths.push_back("index2_bike.bin");
        index_paths.push_back("index3_bike.bin");
...
public:
    ...
    void read_csv(const string& csv_path) {
        ...
        while (getline(csv_file, line)) {
            ...
            if (fields.size() == 20) { // Asegurarse de que hay 20 campos
                ...
                data_file.write(reinterpret_cast<char*>(&record), sizeof(Record1));
            }
        }
        csv_file.close();
        data_file.close();
    ...
    void read_csv2(const string& csv_path) {
        ifstream csv_file(csv_path);
        ofstream data_file(data_path, ios::binary);
        string line;
        getline(csv_file, line); // Ignorar la primera línea (encabezados)
        ...
        while (getline(csv_file, line)) {
            ...
            if (fields.size() == 11) { // Asegurarse de que hay 20 campos
                strncpy(record.id, fields[0].c_str(), sizeof(record.id) - 1);
                strncpy(record.date, fields[1].c_str(), sizeof(record.date) - 1);
                strncpy(record.customer, fields[2].c_str(), sizeof(record.customer) - 1);
                strncpy(record.model, fields[3].c_str(), sizeof(record.model) - 1);
                strncpy(record.price, fields[4].c_str(), sizeof(record.price) - 1);
                strncpy(record.quantity, fields[5].c_str(), sizeof(record.quantity) - 1);
                strncpy(record.storelocation, fields[6].c_str(), sizeof(record.storelocation) - 1);
                strncpy(record.salespersonid, fields[7].c_str(), sizeof(record.salespersonid) - 1);
                strncpy(record.paymentmethod, fields[8].c_str(), sizeof(record.paymentmethod) - 1);
                strncpy(record.customerage, fields[9].c_str(), sizeof(record.customerage) - 1);
                strncpy(record.customergender, fields[10].c_str(), sizeof(record.customergender) - 1);

                data_file.write(reinterpret_cast<char*>(&record), sizeof(Record2));
            }
        }
        csv_file.close();
        data_file.close();
...
pair<RecordTipo, bool> search(char* search_id) override {
    ...
    while (data_index_file.read(reinterpret_cast<char*>(&data_page), sizeof(data_page))) {
        for (size_t i = 0; i < data_page.count; i++) {
            ...
            if (strcmp(search_id, data_page.records[i].id) == 0) {
              ...
              return make_pair(data_page.records[i], true);
            }
        }
...
bool finding(char* search_id){
    ...
    void build_upper_levels() {
        ...
        PageData<RecordTipo>data_index_page;
        PageIndex second_index_page;
    ...
    bool add(RecordTipo new_record) override{

        bool yaesta=finding(new_record.id);
        cout<<yaesta<<endl;
        if(yaesta){
            return false;
        }
        ...
        if (data_page.count == N) {
            PageData<RecordTipo>new_data_page;
    ...
    vector<RecordTipo> rangeSearch(char* start_id,  char* end_id) override{
        fstream root_index_file(index_paths[0], ios::in | ios::binary);
        PageIndex search_page;
        size_t found_page_address = 0;
        bool found = false;
        ...
        sort(records.begin(), records.end(), [](const RecordTipo &a, const RecordTipo &b) {
 ...
 bool remove( char* search_id) override{
    cout<<"eli"<<endl;
    fstream root_index_file(index_paths[0], ios::in | ios::out | ios::binary);
    fstream second_index_file(index_paths[1], ios::in | ios::out | ios::binary);
    fstream data_index_file(index_paths[2], ios::in | ios::out | ios::binary);

    PageIndex root_page, second_page;
    PageData<RecordTipo> data_page;
    streamoff root_page_pos = 0, second_page_pos = 0, data_page_pos = 0;
    bool found = false;

    // Buscar en el índice raíz
    while (root_index_file.read(reinterpret_cast<char*>(&root_page), sizeof(PageIndex))) {
        for (size_t i = 0; i < root_page.count; ++i) {
            if (strcmp(search_id, root_page.keys[i]) <= 0) {
                second_page_pos = root_page.pages[i];
                found = true;
                break;
            }
        }
        if (found) break;
        root_page_pos = root_index_file.tellg();
    }

    if (!found) {
        second_page_pos = root_page.pages[root_page.count - 1];
    }

    // Buscar en el segundo nivel de índice
    found = false;
    second_index_file.seekg(second_page_pos);
    while (second_index_file.read(reinterpret_cast<char*>(&second_page), sizeof(PageIndex))) {
        for (size_t i = 0; i < second_page.count; ++i) {
            if (strcmp(search_id, second_page.keys[i]) <= 0) {
                data_page_pos = second_page.pages[i];
                found = true;
                break;
            }
        }
        if (found) break;
        second_page_pos = second_index_file.tellg();
    }

    if (!found) {
        data_page_pos = second_page.pages[second_page.count - 1];
    }

    // Buscar y eliminar el registro en la página de datos
    data_index_file.seekg(data_page_pos);
    while (data_index_file.read(reinterpret_cast<char*>(&data_page), sizeof(PageData<RecordTipo>))) {
        for (size_t i = 0; i < data_page.count; ++i) {
            if (strcmp(data_page.records[i].id, search_id) == 0) {
                // Eliminar el registro
                for (size_t j = i; j < data_page.count - 1; ++j) {
                    data_page.records[j] = data_page.records[j + 1];
                }
                data_page.count--;

                // Escribir la página actualizada
                data_index_file.seekp(data_page_pos);
                data_index_file.write(reinterpret_cast<char*>(&data_page), sizeof(PageData<RecordTipo>));

                // Actualizar los índices superiores si es necesario
                if (i == 0 && data_page.count > 0) {
                    updateUpperIndices(root_index_file, second_index_file, data_page.records[0].id, data_page_pos);
                }
        ...
        if (data_page.nextPage == -1) break;
        data_page_pos = data_page.nextPage;
        data_index_file.seekg(data_page_pos);
    }
...
void updateUpperIndices(fstream& root_index_file, fstream& second_index_file, const char* new_min_key, streamoff data_page_pos) {
    PageIndex second_page, root_page;
    streamoff second_page_pos = 0, root_page_pos = 0;
        cout<<"actu"<<endl;
    // Actualizar el segundo nivel de índice
    second_index_file.seekg(0);
    while (second_index_file.read(reinterpret_cast<char*>(&second_page), sizeof(PageIndex))) {
        for (size_t i = 0; i < second_page.count; ++i) {
            if (second_page.pages[i] == data_page_pos) {
                strncpy(second_page.keys[i], new_min_key, sizeof(second_page.keys[i]) - 1);
                second_index_file.seekp(second_page_pos);
                second_index_file.write(reinterpret_cast<char*>(&second_page), sizeof(PageIndex));
                
                // Si es la primera clave de la página, actualizar el nivel raíz
                if (i == 0) {
                    updateRootIndex(root_index_file, new_min_key, second_page_pos);
                }
                return;
            }
        }
...
void updateRootIndex(fstream& root_index_file, const char* new_min_key, streamoff second_page_pos) {
    PageIndex root_page;
    streamoff root_page_pos = 0;

    root_index_file.seekg(0);
    while (root_index_file.read(reinterpret_cast<char*>(&root_page), sizeof(PageIndex))) {
        for (size_t i = 0; i < root_page.count; ++i) {
            if (root_page.pages[i] == second_page_pos) {
                strncpy(root_page.keys[i], new_min_key, sizeof(root_page.keys[i]) - 1);
                root_index_file.seekp(root_page_pos);
                root_index_file.write(reinterpret_cast<char*>(&root_page), sizeof(PageIndex));
                return;
            }
        }
        root_page_pos = root_index_file.tellg();
    ...
    for (size_t i = 0; i < index_paths.size(); ++i) {
        ...
        if (i < 2) {  // Para los niveles de índice
            ...
            PageData<RecordTipo>page;
            while (index_file.read(reinterpret_cast<char*>(&page), sizeof(PageData<RecordTipo>))) {
                for (size_t j = 0; j < page.count; ++j) {
                    cout << "ID: " << page.records[j].id << endl;
                }
