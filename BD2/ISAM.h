...
template<typename R>
class ISAM: public SeleccionarMetodo<R> {
public:
    ISAM(const string& csv_path,bool tipo) : data_path("data.bin") {
        // Rutas de índice
...
public:
    ...
    vector<R> search(const char* search_id) {
    ...
    bool add(R new_record) {
        fstream root_index_file(index_paths[0], ios::in | ios::binary);
        PageIndex search_page;
        size_t found_page_address = 0;
        bool found = false;

        // Buscar la página en el índice raíz
        ...
        // Buscar la página en el segundo índice
        ...
        for (int i = 0; i < data_page.count; i++) {
            if (strcmp(data_page.records[i].id, new_record.id) == 0) {
                // El registro ya existe, no se debe insertar
                root_index_file.close();
                second_index_file.close();
                data_index_file.close();
                return false;  // Retornar false en caso de duplicado
            }
        }
        ...
        if (data_page.count == N) {  // Página llena
            PageData<R> new_data_page;
        ...
        } else {  // Insertar en la página existente
    ...
    vector<R> range_search(const char* start_id, const char* end_id) {
        fstream root_index_file(index_paths[0], ios::in | ios::binary);
        PageIndex search_page;
        size_t found_page_address = 0;
        bool found = false;
        ...
        sort(records.begin(), records.end(), [](const R &a, const R &b) {
    ...
    void print_index_files() {
        for (size_t i = 0; i < index_paths.size(); ++i) {
            ...
            if (i < 2) {  // Para los niveles de índice
                ...
                PageData<R>page;
                while (index_file.read(reinterpret_cast<char*>(&page), sizeof(PageData<R>))) {
                    for (size_t j = 0; j < page.count; ++j) {
...
void pruebaISAM(string csv_path){
    ...
    vector<R> range_result = isam.range_search(start_id, end_id);
